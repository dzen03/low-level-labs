...
struct bmp_header create_header(uint32_t width, uint32_t height) {
  ...
  return (struct bmp_header) {19778, (height * row_size_byte) + sizeof(struct bmp_header), 0,
                              sizeof(struct bmp_header), 40,
                              width, height, 1, sizeof(struct pixel) * 8, 0, height * row_size_byte,
                              2834, 2834, 0, 0};
}
