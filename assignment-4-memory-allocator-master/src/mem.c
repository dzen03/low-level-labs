...
static struct region alloc_region  ( void const * addr, size_t query ) {
  ...
  void* mapped_addr = map_pages(addr, size, MAP_FIXED);

  mapped_addr = (mapped_addr == MAP_FAILED ? map_pages(addr, size, 0) : mapped_addr);
  mapped_addr = (mapped_addr == MAP_FAILED ? NULL : mapped_addr);

  struct region region = {.addr = mapped_addr, .extends = mapped_addr==addr, .size = size};

  if (!region_is_invalid(&region))
  {
    block_init(mapped_addr, (block_size) {.bytes = region.size}, NULL);
  }
  return region;
}
